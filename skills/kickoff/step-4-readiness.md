...
Print the Step 4 exit one-liner. Required outputs: readiness decision (`spec` or `blueprint` path), rationale, Work Contract status when derived or approved, spec lifecycle status/artifact path if spec ran, workflow policy level when configured, and context packet to carry forward.
