...
- `crust_seam` combines a validation pass (shape) with a `binary` probe (presence/version) and, when `ok`, a `.crust/` freshness diff. Do not deep-invoke any other crust subcommand from doctor; the freshness check is the one crust invocation doctor performs.
- Report which of the five delegated seams (gate selection, action-policy verdict + placement, workflow normalization, run-ledger writes) are currently crust-backed vs falling back, based on `crust_seam` status and `mode`.
