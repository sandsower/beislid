...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `nopal_seam`, validate shape and run the exact binary probe from `probe-semantics.md` and `nopal-seam-protocol.md`.
     ...
     When the probe is `ok`, run `nopal import beislid-workflow --source .beislid/workflow.md --output-dir .nopal --check --json`.
     Report `beislid_import_drift` with a `nopal import beislid-workflow --source .beislid/workflow.md --output-dir .nopal --write --overwrite --json` remediation instead of editing modules directly.
