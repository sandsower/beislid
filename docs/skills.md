...
- `kickoff`: fetch a ticket and route to the right workflow; may derive a Work Contract, surface the active workflow policy level, and write a configured context checkpoint.
...
- `verify`: no success claims without evidence; surfaces the active workflow policy level in the report when configured.
...
- `ready-for-review`: review-ready PR flow driven by `workflow.md`; runs `review`, surfaces the active workflow policy level, and runs the configured final check before new PR creation.
...
- `doctor`: audit `.beislid/workflow.md`, including the workflow policy level, and probe each configured capability.
...
| `kickoff`       | Ticket router               | Starting work from an existing ticket branch; reads `<repo>/.beislid/workflow.md`; may derive a Work Contract, surface the active workflow policy level, and write configured context checkpoints | `spec`, `break-spec`, or `blueprint`                          |
...
| `ready-for-review`       | Review handoff flow               | Branch is ready for a new PR or an existing PR update; reads `<repo>/.beislid/workflow.md`, probes capabilities lazily, surfaces the active workflow policy level, and can summarize configured ship-time planning artifacts | new PR or pushed update                                       |
...
| `doctor`        | Config audit                | You edited `workflow.md` or want a capability check before running orchestrators; includes workflow policy level handling | refreshed probe cache plus prose audit                        |
