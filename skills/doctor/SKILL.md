...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `workflow_policy`, validate rather than probe workflow posture. `level` must be one of `advisory`, `standard`, `strict`, or `regulated`; absent block means the default standard posture. Record valid config as `status: ok`, `probe_kind: validation`, and a concise value naming the active level. Surface the level in doctor prose, but do not let it change proof or action-policy validation.
