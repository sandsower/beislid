...
This repo dogfoods the crust seam. `.crust/` is committed and generated by `crust import beislid-workflow --write`; keep it in sync when `.beislid/workflow.md` changes (`crust import beislid-workflow --source .beislid/workflow.md --json` in preview mode diffs cleanly against the committed files).
