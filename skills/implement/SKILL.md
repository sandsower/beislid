...
If `beislid:guides` is configured, load any guide whose `stage` is `implement` or whose selectors match the files or areas under implementation. Guides are feedforward context, not proof: they help the plan stay grounded, but they never replace the approved design.
