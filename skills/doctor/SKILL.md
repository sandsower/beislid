...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `crust_seam`, validate shape then run the `binary` probe kind per `probe-semantics.md` and `crust-seam-protocol.md`. `mode` must be `prefer`, `require`, or `off`; `binary` and `min_version` (when present) must be non-empty strings. `mode: off` records `status: ok` without probing. Otherwise probe `command -v <binary>` then `<binary> info --json`, confirming `kind: crust.info/v1` and reading `version`, `commit`, and `capabilities[]`; `missing` when the binary isn't found. When `info --json` fails (older crust, clap exit 2 on the unknown subcommand), fall back to `<binary> --version`'s plain-text triple parse and record `status: ok` with `probe_mode: legacy` instead of failing outright; `failed` only when neither technique parses a version or the resolved version is below `min_version`. When `ok`, also re-run `crust import beislid-workflow --source .beislid/workflow.md --json` in preview mode and diff its `outputs[].content` against the committed `.crust/*.jsonc` files per module; report drift with a `crust import beislid-workflow --write --overwrite --json` remediation rather than editing `.crust/` directly. `mode: prefer` with the binary missing is a graceful fallback note (install story: `cargo build --release -p crust-cli` from the crust repo; no published release yet), not a failure. `mode: require` with the binary missing is a real gap - name which seams would otherwise delegate to crust. When `probe_mode: rich` but a seam's required capability family is absent from `capabilities[]`, report it precisely as "crust present but missing `<capability>`" rather than a generic version-cutoff warning.
...
Also check the crust seam:

- `crust_seam` combines a validation pass (shape) with a `binary` probe (`crust info --json`, presence/version/capabilities) and, when `ok`, a `.crust/` freshness diff. Do not deep-invoke any other crust subcommand from doctor; the freshness check is the one crust invocation doctor performs beyond the probe itself.
- Report which of the five delegated seams (gate selection, action-policy verdict + placement, workflow normalization, run-ledger writes) are currently crust-backed vs falling back, based on `crust_seam` status, `mode`, and capability membership in `capabilities[]` rather than a version cutoff.
- When `probe_mode: legacy` (older crust, `info` subcommand missing), narrate that capability membership can't be checked and every seam falls back to its documented legacy path until crust is upgraded - this is a degraded `ok`, not a failure, and doctor should still name the install/upgrade story.
- `review_policy`, `ready_for_review`, `clean_eval`, and `babysit` have no crust v1 module; never suggest they would be crust-backed regardless of `crust_seam` status.
