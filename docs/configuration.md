...
Closeout stages run in a fixed order: `merge`, then `memento`, then `retro`, then `cleanup`.

`cleanup` is the only stage with an inherited default. Leave `closeout.cleanup.mode` unset and it follows the effective `closeout.merge.mode`, so a repo that already auto-merges gets cleanup with no config change; set it explicitly only when cleanup should differ from merge. Cleanup runs last and only after a successful merge. It proves no branch content is unlanded — a clean working tree plus no overlap between the paths this branch touched and the paths that still differ from the freshly fetched default branch, which is what works when the merge was a squash and the default branch has since moved on unrelated paths — then closes the ticket through the configured `ticket_update` close channel (`close_tool` / `close_command`) and assigns it to the PR author, deletes the remote branch when `closeout.merge.delete_branch` is set and the merge did not already remove it, and reports the worktree path and branch as ready for removal. It never removes the worktree or the local branch: the agent is running inside that worktree, so removing it would kill the session mid-report. The supervising session removes it afterwards. Any unlanded work stops cleanup and is reported, never discarded.

Defaults are conservative when this block is absent: goal has no token budget, `review-response` is used for feedback handling, configured gates run before pushes, and merge/memento/retro closeout steps are off with cleanup following merge. Set `loop.use_review_response: false` only when you want `babysit` to stop with a feedback summary instead of fixing, replying, committing, or pushing automatically. `auto` closeout removes routine prompts only where action policy allows the side effect; if policy asks or denies, the skill asks or stops.
...
The bundle and per-slice shape checks above are declared as data, not hand-written conditionals: `schemas/approved-slice-plan-export-v0.schema.json` for `bundle.json` and `schemas/execution-envelope-v0.schema.json` for each per-slice manifest.
`scripts/schema_check.py` is the stdlib-only interpreter that enforces them, and it supports exactly `type`, `required`, `properties`, `enum`, `items`, `minimum`, and `pattern` — no `oneOf`, `additionalProperties`, or `minItems`.
Graph-level and cross-field semantics that the subset cannot express — cycle detection, parallel-group transitive closure, supersedes/version pairing, slice_id-matches-filename, and the prompt-or-body fallback — stay in `scripts/validate_export.py` as code; each schema's `$comment` header names exactly which checks remain code-side.
`scripts/check_contract_schema_consistency.py` keeps the schemas' declared fields and enum vocabularies in sync with this section of the doc.
...
Default `plans/` paths are intentionally discoverable by downstream skills. Custom paths stay rediscoverable later through same-session handoff context and the latest pointer when one exists. The workflow-configured path template is a best-effort fallback, not a guarantee, because placeholders may have been derived from approved titles or other session-local context; downstream skills still fall back to the default `plans/` search when those sources are unavailable. Planning artifacts are also checkpoint-compatible state seeds: a fresh context may use an approved structure/spec/design artifact as its primary input when it captures enough context for the next skill.
...
Planning workflow routing is conservative. `blueprint` should use Lavish only when a visual plan, implementation-options comparison, architecture/data-flow diagram, or risk/test matrix materially improves design approval. `poke-holes` should use Lavish only for decision trees, branching tradeoffs, risk maps, or diagram-backed stress tests. Simple linear questions and normal approval gates stay in Markdown/chat, and visual choices or approvals must be copied into the canonical Markdown/chat record before downstream routing.
...
When visual routing is active for a workflow, the reusable protocol's canonical source lives in `.beislid/visual-surface-protocol.md`; workflow skills may expose it through a per-skill readable auxiliary copy for project-local installs. It defines how to write supplemental Lavish-ready HTML review surfaces, the Beislið/provider boundary, graceful Markdown/chat fallback behavior, planning workflow surfaces, Show Me deck routing, artifact_retention, the `BEISLID_VISUAL_PROMPT_V1` prompt envelope, and the typed `BEISLID_VISUAL_FEEDBACK_V1` validation contract. Workflows should load that protocol only when repo-level `beislid:visual_surfaces` config makes the effective mode active; user-level plugin state alone is not enough.

Typed gate feedback is separate from freeform annotations. A workflow may consume a visual decision only after the typed payload validates for the current workflow/action and normalizes to an accepted decision; unknown action, unknown decision, duplicate fields, multiple typed payloads, malformed payload, freeform-only feedback, or unavailable parser support must continue through manual Markdown/chat review. The repository ships a dependency-free helper exposed as `beislid visual-feedback normalize`, for hosts that want a shared parser/normalizer without taking a Lavish runtime dependency or relying on the current working directory. Its normalized event includes `manual_review` fallback reasons and `canonical_update_required` so the accepted visual decision or fallback reason can be copied into the canonical Markdown/chat record before any downstream routing. In addition to the `spec` approve/revise gate, v1 recognizes `blueprint` approve/revise/choose and `poke-holes` resolved/revise/choose planning gates; `choose` requires `selected_option` and never counts as implementation approval by itself. For Phase 1 compatibility, the helper also accepts the old flat `workflow`/`action`/`decision` payload shape when the schema token was omitted and the gate is otherwise unambiguous.
...
- `babysit`: PR review source/update path, configured gates/scopes/gate sets, action policy, babysit closeout policy, the `ticket_update` close channel and `branch_pattern` used by the cleanup stage, and goal-support disclosure.
...
It writes `<project>/.beislid/project-install.json` with source path, version/commit, mode, targets, and install counts. In copy mode, each copied skill dir also gets a `.beislid-owner.json` marker with `owner: beislid`, `mode: copy`, the skill name, host, source path, version, and commit. Reruns refresh existing Beislið-owned copied dirs using either the marker or manifest as ownership evidence. `beislid repair project [path]` restores an existing project install without guessing a mode or rewriting the managed `.gitignore` block; use `beislid install project [path]` when you are bootstrapping or intentionally changing mode/targets. `beislid status project [path]` reports missing skills per host and exits non-zero when any supported host is incomplete. Unmarked project files or directories are skipped and are not clobbered, even with `--force`.
