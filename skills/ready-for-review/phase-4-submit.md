...
3. Include repo, branch, base, ticket id or `none`, PR URL if any, phase path (`new-pr-fast-path` when used), aux loaded, transcript path/unavailable reason, gates including parallel mode, review/fresh-eyes or combined-review status including cancellation/partial output, accepted risks, side effects, host, timestamp, and duration if known.
