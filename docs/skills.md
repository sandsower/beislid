...
- Lavish visual surfaces: optional supplemental HTML review/planning surfaces for workflows such as `spec` when a repo configures `beislid:visual_surfaces`. Markdown/chat artifacts remain canonical. Use `beislid plugin enable lavish`, `beislid plugin status lavish`, and an optional pinned `--command` to manage local plugin state; missing/disabled state, missing `npx`, failed deep checks, declined prompts, and runtime failures fall back to Markdown/chat. See [Configuration: Visual surfaces](./configuration.md#visual-surfaces).
