...
2. **Requirements check** — if product behavior or acceptance criteria are unclear, stop and route to `spec` with the missing questions. When a Work Contract is present, also check its status, unknowns/human decisions, shallow `scope_classification`, reserved `proof_requirements`, and reserved `slice_plan`/`children` before designing. Broad/project work should not jump directly to scaffolding by default. Do not patch over vague requirements with implementation guesses.
