...
- `break_spec_approved`, `blueprint_approved`, `kickoff_context_ready`, and `implementation_plan_created` support only `type: artifact` in P0. `spec_approved` supports `type: artifact` and `type: tracker`; tracker actions update the ticket body through `ticket_update` and are gated by `ticket.update` policy. Non-supported actions under the other events are reserved; warn that skills will skip them.
