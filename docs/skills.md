...
| `implement`     | Execution planning          | Implementation design is approved; may coordinate controlled subagents, consume approved execution envelopes, and write configured implementation-plan checkpoints | code changes + `verify`                                       |
...
| `review`        | Review primitive            | Local or supplied diff needs first-pass findings, optional spec/code-quality reviewer passes, and a readiness verdict      | caller decides: fix, post, loop, or submit                      |
...
| `rinse`         | Review orchestrator         | You want an approved review/fix/verify loop, including delegated-review reconciliation when useful                         | `fresh-eyes` or `ready-for-review`                                     |
