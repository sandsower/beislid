...
`beislid:agent_guidance` is validated as pointer-only routing metadata for visible host-native startup guidance. Doctor checks shape only: optional `default` is a repo-relative path with no `..`; optional `hosts` is a map from host ids to repo-relative paths; known v1 host ids are `pi`, `claude-code`, and `codex`; optional `edit_policy` must be `prompt`. Unknown host ids warn as reserved rather than fail when their paths are valid. Doctor may check whether configured files exist and whether known host mappings look native, but missing files are guidance warnings, not config failures. The block must not contain the guidance content itself.

### skill_guidance validation

`beislid:skill_guidance` is validated as pointer-only project guidance overlays for consuming Beislið skills. Doctor checks top-level keys (`all` plus known skill names; unknown names warn as likely typos), each value is a list, and every entry has a safe repo-relative `.md` `path`, optional `mode` (`read-if-present` or `must-read`, default `read-if-present`), and optional `when` (`always` in v1). For `must-read`, the referenced file must exist, be readable, and be non-empty; missing, unreadable, or empty files are failures. For `read-if-present`, a missing file is non-blocking guidance. Doctor never creates guidance files.
