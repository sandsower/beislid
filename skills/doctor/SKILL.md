...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `agent_guidance`, validate as pointer-only host guidance routing. `default` and every `hosts.*` value must be repo-relative paths with no `..` segments; `edit_policy` must be `prompt` when present. Known host ids are `pi`, `claude-code`, and `codex`; unknown host ids warn as reserved rather than fail when paths are valid. Record valid config as `status: ok`, `probe_kind: validation`, and a concise value naming default path, host mappings, and any missing target-file guidance. Warn if the block appears to contain guidance prose instead of routing metadata.
   - For `skill_guidance`, validate as pointer-only project guidance overlays. Top-level keys are `all` plus known Beislið skill names; unknown names warn as likely typos. Values must be lists of entries with safe repo-relative `.md` `path`, optional `mode` (`read-if-present` or `must-read`, default `read-if-present`), and optional `when` (`always` in v1). `must-read` files must exist, be readable, and be non-empty; missing/unreadable/empty targets mark `skill_guidance` failed. Missing `read-if-present` targets are non-blocking guidance. Record valid config as `status: ok`, `probe_kind: validation`, and a concise value naming overlay count and must-read count.
...
Also check agent and skill guidance:

- `agent_guidance` is validation-only and stores routing metadata only. Doctor should not create or edit target files. Known host mappings should be narrated as visible startup guidance targets; missing files are guidance warnings unless another rule requires them. If users put long instructions directly in `workflow.md`, warn that `workflow.md` should hold pointers/policy, not guidance content.
- `skill_guidance` is validation-only. Doctor should validate `all` and known skill-name keys, safe `.md` paths, modes, and `when: always`. A `must-read` target that is missing, unreadable, or empty is a config failure because consuming skills fail closed. A missing `read-if-present` target is non-blocking. Unknown skill keys are warnings because they are likely typos and no consuming skill may load them.

Also check visual surfaces:

- `visual_surfaces` is validation-only; doctor does not deep-invoke Lavish or spend npm/network/cache by running the provider command. Invalid provider/mode/artifact_root/workflow override shape is a config failure.
- Valid provider is `lavish-axi`. Valid modes are `off`, `suggest`, `prompt`, and `auto`; per-workflow overrides use the same enum. `artifact_root` must be repo-relative and cannot contain `..` segments.
