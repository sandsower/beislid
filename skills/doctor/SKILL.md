...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `review_policy`, validate AgenticReviewer final-review policy rather than probing PR providers. `agentic_reviewer.mode` must be `opt_in_final_review`; optional `provider`, `label`, and `description_keyword` must be non-empty strings; risk enum values are `low`, `medium`, `high`; path lists are string lists; thresholds are positive integers. Missing block is valid/backward-compatible. Legacy `coderabbit` may be warned as provider-specific legacy config.
