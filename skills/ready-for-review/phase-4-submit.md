...
4. If a run ledger is active, record ledger end-state explicitly: use `beislid run-ledger finalize` on successful PR handoff, and use `beislid run-ledger interrupt` with abort context on abort paths. Keep the memory marker for smoke compatibility.
