...
check_contains "docs/worktree-isolation.md" 'Every parallel mutating agent must have a different dedicated worktree and branch before dispatch.'
check_contains "docs/worktree-isolation.md" 'A worktree does not isolate external mutable state.'
check_contains "skills/implement/SKILL.md" 'Every parallel mutating agent must have a different dedicated worktree and branch before dispatch'
check_contains "skills/implement/SKILL.md" 'If the host cannot provision distinct worktrees, execute the batch sequentially in one owned worktree'
