...
4. If a run ledger is active: `finalize` only after successful PR handoff (`crust ledger finalize --json` when the crust seam started the run, else `beislid run-ledger finalize`); on abort, record `crust ledger interrupt` or `beislid run-ledger interrupt` with context, matching whichever CLI initialized the run (`crust-seam-protocol.md`).
