...
- `ticket_update` — shared by kickoff, review-response, and babysit. Fields: `type` (`mcp` / `cli`); comment channel is used for kickoff plan comments and review-response ticket replies (`comment_tool` when `type: mcp`, `comment_command` when `type: cli`); issue channel is optional for review-response child tickets (`issue_tool` / `issue_command`) and `spec_approved` tracker body posts; close channel is optional and used only by babysit's `cleanup` closeout stage (`close_tool` when `type: mcp`, `close_command` when `type: cli`). MCP probes treat the configured comment/issue tool as canonical, but a host-adapter alias for an equivalent tool may satisfy it; probe output should distinguish exact from alias-satisfied hits. CLI comment commands use `{id}` + `{body_file}` placeholders; issue commands use `{title_file}` + `{body_file}`; close commands use `{id}` plus optional `{state}` and `{assignee}`. An MCP `close_tool` must be a tool that can set an existing ticket's state and assignee, not an issue-creation tool. When no close channel is configured, cleanup falls back to the issue channel only if it can express a state and assignee change for an existing ticket, and otherwise prints the exact manual close step instead of inventing a command. Orchestrators write temp files and substitute file paths — never interpolate raw user-authored body/title text into shell commands.
...
Closeout mode values are `off`, `ask`, and `auto`. `off` disables that closeout step. `ask` stops for explicit approval. `auto` proceeds without an extra babysit prompt only when action policy allows the specific side effect; policy `ask` still asks and policy `deny` still stops. Invocation args can override config for a single run, for example `stop when green`, `don't merge`, `merge then stop`, `skip memento`, `skip retro`, or `skip cleanup`.

Closeout stages run in a fixed order: `merge`, then `memento`, then `retro`, then `cleanup`. `closeout.cleanup.mode` is the one stage with an inherited default: when the key is absent it follows the effective `closeout.merge.mode`, so a repo that already auto-merges gets cleanup without a config change. Cleanup is reachable only after a successful merge. It proves no branch content is unlanded, closes the ticket through the configured `ticket_update` close channel, deletes the remote branch when `closeout.merge.delete_branch` is set and the ref survived the merge, and reports the worktree path and branch as ready for removal. It never removes the worktree or the local branch itself.
