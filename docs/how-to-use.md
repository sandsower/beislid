...
4. If you want repo knowledge loaded progressively before a step, configure `beislid:guides` in `.beislid/workflow.md`. Guides are feedforward Markdown artifacts, not readiness gates; they help `kickoff`, `spec`, `blueprint`, `implement`, and review-side workflows preload the right docs for the current stage or touched area.

5. Run `/doctor` to validate the repo config shape. Doctor and normal `plugin status` do not deep-invoke Lavish. Use `beislid plugin status lavish --check` only when you deliberately want to run the configured command and accept possible npm/network/cache activity.
