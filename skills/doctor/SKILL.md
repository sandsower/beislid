...
4. **Probe/validate.** Otherwise, dispatch on `type` or the key's canonical probe kind per `probe-semantics.md`:
   ...
   - For `guides`, validate the registry and the referenced Markdown paths rather than probing a capability. `name`, `path`, and `stage` are required; `path` must be a relative repo-local `.md` file that exists; optional `scopes` / `paths` / `exclude` are glob lists; supported `stage` values are `kickoff`, `spec`, `blueprint`, `implement`, and `review`. Record valid config as `status: ok`, `probe_kind: validation`, and a concise value naming guide count, stage count, and path health. Guide loading is feedforward metadata, not readiness proof.
