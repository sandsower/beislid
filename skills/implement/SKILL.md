...
If the handoff includes an approved `execution-envelope-v0`, treat its autonomy boundary as execution context: `allow` lists pre-approved work, `ask` creates human approval stops before continuing, and `deny` is out of scope. Do not reinterpret an envelope as a runtime service, parser, durable run store, or replacement for Work Contract requirements.
